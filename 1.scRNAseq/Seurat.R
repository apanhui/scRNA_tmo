...
obj <- StatFeatures(obj, parameter$marker$mito_list, col.name = "percent.mito", stat_pct = T, add_to_pdata = T)
obj <- StatFeatures(obj, parameter$marker$plastid_list, col.name = "percent.plastid", stat_pct = T, add_to_pdata = T)
obj <- StatFeatures(obj, parameter$marker$rrna_list, col.name = "percent.rrna", stat_pct = T, add_to_pdata = T)
obj <- StatFeatures(obj, parameter$marker$expected, col.name = "expected.marker")
obj <- StatFeatures(obj, parameter$marker$excluded, col.name = "exclude.marker")
obj@misc[["more.marker"]] <- FindFeaturesID(obj, parameter$marker$more, unlist = T)
...
obj <- DoNormalization(obj, parameter,
...
if ( ! is.null(parameter$integration$method) ) {
	if ( length(table(obj[["orig.ident"]])) > 1 ) {
		...
		PlotCluster(obj, reduction = paste0('umap_', assay), outpref = "UMAP_before" )
		PlotCluster(obj, reduction = paste0('tsne_', assay), outpref = "tSNE_before" )
		if ( ! is.null(parameter$Groups) ) {
			PlotCluster(obj, reduction = paste0('umap_', assay), outpref = "UMAP_before.groups", split.by = "Groups", p1.group.by = "Groups" )
			PlotCluster(obj, reduction = paste0('tsne_', assay), outpref = "tSNE_before.groups", split.by = "Groups", p1.group.by = "Groups" )
		}
